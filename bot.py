...
async def adduser(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not context.args:
        await update.message.reply_text("⚠️ Errore: Devi specificare un nome.\nEsempio: `/adduser Pippo`")
        return
    
    nuovo_utente = context.args[0]
    utenti = carica_utenti()
    
    if nuovo_utente in utenti:
        await update.message.reply_text(f"⚠️ `{nuovo_utente}` è già presente nella lista.")
        return
...
async def deluser(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not context.args:
        await update.message.reply_text("⚠️ Errore: Devi specificare un nome.\nEsempio: `/deluser Pippo`")
        return
    
    utente_da_rimuovere = context.args[0]
    utenti = carica_utenti()

    if utente_da_rimuovere not in utenti:
        await update.message.reply_text(f"⚠️ Utente `{utente_da_rimuovere}` non trovato.")
        return
...
async def reset_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    keyboard = [[
        InlineKeyboardButton("✅ SÌ, RESETTA TUTTO", callback_data='reset_confirm'),
        InlineKeyboardButton("❌ NO, ANNULLA", callback_data='reset_cancel')
    ]]
...
async def gestisci_pulsanti_classifica(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not classifica_ordinata:
        testo_risposta += "Nessun dato da mostrare."
    else:
